...
MODULE_DIR = PROJECT_ROOT / "module"  # 模块目录
...
class Packager:
    ...
    def _check_requirements(self) -> bool:
        ...
        if not MODULE_DIR.exists():
            print(f"错误: 未找到模块目录: {MODULE_DIR}")
            return False
            
        if not (MODULE_DIR / "module.prop").exists():
            print("错误: 模块目录中未找到module.prop文件")
            return False
...
class FileManager:
    ...
    def fix_line_endings():
        ...
        for ext in LINE_ENDING_FILES:
            for file_path in MODULE_DIR.rglob(f"*{ext}"):
    ...
    def _convert_line_endings(file_path: Path) -> bool:
        ...
        try:
            ...
            if b'\r\n' in content:
                ...
                print(f"  转换: {file_path.relative_to(MODULE_DIR)}")
                return True
        except OSError:
            pass
        return False
