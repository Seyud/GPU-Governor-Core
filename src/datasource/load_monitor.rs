...
use crate::utils::file_operate::{read_file, check_read, check_read_simple};
use crate::utils::file_status::{write_status, get_status};
...
fn kernel_ged_load() -> Result<i32> {
    ...
    if parts.len() >= 3 {
        if let Ok(idle) = parts[2].parse::<i32>() {
            ...
            return Ok(if 100 - idle == 0 { module_ged_load()? } else { 100 - idle });
        }
    }
...
fn kernel_debug_ged_load() -> Result<i32> {
    ...
    if parts.len() >= 3 {
        if let Ok(idle) = parts[2].parse::<i32>() {
            ...
            return Ok(if 100 - idle == 0 { kernel_ged_load()? } else { 100 - idle });
        }
    }
...
fn kernel_d_ged_load() -> Result<i32> {
    ...
    if parts.len() >= 3 {
        if let Ok(idle) = parts[2].parse::<i32>() {
            ...
            return Ok(if 100 - idle == 0 { kernel_debug_ged_load()? } else { 100 - idle });
        }
    }
...
fn mali_load() -> Result<i32> {
    ...
    if let Some(pos) = buf.find('=') {
        if let Ok(load) = buf[pos+1..].trim().parse::<i32>() {
            debug!("mali {}", load);
            return Ok(if load == 0 { kernel_d_ged_load()? } else { load });
        }
    }
...
fn debug_dvfs_load_func() -> Result<i32> {
    ...
    // Parse busy/idle/protm times
    let mut busy_time = 0;
    let mut idle_time = 0;
    let mut protm_time = 0;
    ...
    if parts.len() >= 3 {
        if let (Ok(busy), Ok(idle), Ok(protm)) = (
            ...
            let (prev_busy, prev_idle, prev_protm) = unsafe {
                (PREV_BUSY, PREV_IDLE, PREV_PROTM)
            };
...
pub fn utilization_init() -> Result<()> {
    ...
    info!("{}: {}", KERNEL_DEBUG_LOAD, check_read(KERNEL_DEBUG_LOAD, &mut is_good));
    info!("{}: {}", KERNEL_D_LOAD, check_read(KERNEL_D_LOAD, &mut is_good));
    
    // Method 4: Read From /proc/gpufreq
    info!("Testing gpufreq Driver...");
    info!("{}: {}", GPU_FREQ_LOAD_PATH, check_read(GPU_FREQ_LOAD_PATH, &mut is_good));
    
    // Method 5: Read From Mali Driver
    info!("Testing mali driver ...");
    info!("{}: {}", PROC_MTK_LOAD, check_read(PROC_MTK_LOAD, &mut is_good));
    info!("{}: {}", PROC_MALI_LOAD, check_read(PROC_MALI_LOAD, &mut is_good));
    
    // Method 6: Read precise load from Mali Driver
    info!("{}: {}", DEBUG_DVFS_LOAD, check_read(DEBUG_DVFS_LOAD, &mut is_good));
    info!("{}: {}", DEBUG_DVFS_LOAD_OLD, check_read(DEBUG_DVFS_LOAD_OLD, &mut is_good));
